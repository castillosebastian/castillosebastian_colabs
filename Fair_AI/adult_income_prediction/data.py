...
def convert_df_to_tensor(data_X_df, data_Y_df):
    ...
    data_X_df = data_X_df.astype(float)
    data_Y_df = data_Y_df.astype(int)  # or .astype(int) if your labels are integers

    data_X = torch.tensor(data_X_df.values).float()
    data_Y = torch.tensor(data_Y_df.values).long()  # or .long() if your labels are integers
